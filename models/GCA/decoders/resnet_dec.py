...
from models.GCA.ops import SpectralNorm
...
class ResNet_D_Dec(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, large_kernel=False, late_downsample=False, layer_multi=[1, 1, 1]):
        ...
        self.tanh = nn.Tanh()
        self.layer1 = self._make_layer(block, 256, layers[0], stride=2, inplane_multi=layer_multi[0])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, inplane_multi=layer_multi[1])
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2, inplane_multi=layer_multi[2])
    ...
    def _make_layer(self, block, planes, blocks, stride=1, inplane_multi=1):
        ...
        layers = [block(int(self.inplanes), planes, stride, upsample, norm_layer, self.large_kernel)]
