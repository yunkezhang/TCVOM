...
class GuidedCxtAtten(nn.Module):
    ...
    def forward(self, f, alpha, unknown=None, ksize=3, stride=1, fuse_k=3, softmax_scale=1., training=True):
        ...
        y = torch.cat(y, dim=0).contiguous() # back to the mini-batch
        # a pure sanity check, since the line below that originally
        # has no use (the returned tensor is never used)
        assert list(y.shape) == raw_int_alpha, '{} {}'.format(y.shape, raw_int_alpha)
