...
def train(epoch, trainloader, steps_per_val, base_lr,
    ...
    for i_iter, dp in enumerate(trainloader):
        ...
        if i_iter % image_freq == 0 and local_rank <= 0:
            write_image(image_outdir, vis_images, i_iter+cur_iters)
...
def validate(testloader, model, test_size, local_rank):
    ...
    with torch.no_grad():
        ...
        for batch in iterator:
            def handle_batch():
                fg, bg, a, _ = batch      # [B, 3, 3 or 1, H, W]
                out = model(a, fg, bg)
                L_alpha = out[0].mean()
                L_comp = out[1].mean()
                L_grad = out[2].mean()
            ...
            ave_loss.update(reduced_loss.detach().item())
    if local_rank <= 0:
...
def main(cfg_name, cfg, local_rank):
    ...
    # build model
    model = FullModel(model=cfg.MODEL, agg_window=cfg.AGG_WINDOW, eps=0, \
        freeze_backbone=cfg.TRAIN.FREEZE_BACKBONE)
    if local_rank <= 0 and cfg.MODEL[4:7] == 'res':
        encoder_weight_name = 'resnet34-333f7ec4.pth' \
            if cfg.MODEL[7:9] == '34' else 'resnext50_32x4d-7cdf4587.pth'
        if os.path.exists(encoder_weight_name):
            encoder_weight = torch.load(encoder_weight_name, map_location='cpu')
            dummy = torch.zeros_like(encoder_weight['conv1.weight'][:, :1, :, :])
            encoder_weight['conv1.weight'] = torch.cat([encoder_weight['conv1.weight'], dummy], dim=1)
            missing_keys, unexpected_keys = model.NET.encoder.load_state_dict(encoder_weight, strict=False)
            logger.info('Missing keys: ' + str(sorted(missing_keys)))
            logger.info('Unexpected keys: ' + str(sorted(unexpected_keys)))
            logger.info("=> loaded checkpoint from {} ImageNet pretrain weight.".format(encoder_weight_name))
    torch_barrier()
    ...
    if cfg.TRAIN.FREEZE_BACKBONE:
        params_dict = {k: v for k, v in model.named_parameters() \
            if v.requires_grad and (k[11:] in missing_keys and not k[11:].startswith('semantic'))}
    else:
        params_dict = {k: v for k, v in model.named_parameters() \
            if v.requires_grad and not k[11:].startswith('semantic')}
